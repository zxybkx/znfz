...
class Question extends PureComponent {
  ...
  render() {
    const {dispatch, match, zcjd, loading, globalLoading, znfz} = this.props;
    const {params: {id}} = match;
    const {ajxx, categories, coords, docViewVisible} = zcjd;
    const {docTree} = znfz;
    const {fullScreen, currentList} = this.state;

    const listProps = {
      fullScreen, loading, ajxx, stage, dispatch, docTree, list: currentList,
