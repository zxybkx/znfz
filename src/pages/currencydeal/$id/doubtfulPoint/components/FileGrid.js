...
export default class FileGrid extends PureComponent {
  ...
  getGridColumns = (list) => {
    ...
    const columns = [{
      Header: props => {
        return (
          <div style={{backgroundColor: '#e6f7ff', height: '30px', borderRight: '1px solid #ddd'}}>
      ...
      Cell: ({value, original, index}) => {
        ...
        return (
          <Tooltip placement='right' title={exist ? value : `${value}(文书不存在)`} tooltip>
            <a onClick={() => exist ? this.onClick(index) : null}
               style={{color: exist ? '#1890ff' : 'red'}}>{exist ? value : `${value}(文书不存在)`}</a>
          </Tooltip>
        );
      },
    }];
        ...
        Cell: ({value, original}) => {
          ...
          if (has) {
            if (value && value.length > 0) {
              ...
              return (
                <Tooltip title='未提取到'>
                  <span style={{color: 'red'}}>未提取到</span>
                </Tooltip>
              )
            }
          } else {
  ...
  dealData = (item) => {
    let zz = _.flattenDeep(_.map(item,(val)=>{
      return  _.values(val);
    }));

    let newFieldVal = zz.length>0 ? zz.map((item)=>{
      if(item&&item.fieldvalue){
        return item.fieldvalue
      }
    }):[];

    return _.compact(newFieldVal);
  };
    ...
    _.map(list, file => {
      ...
      _.map(file.contents, (item) => {
        if (item.fieldvalue && Array.isArray(item.fieldvalue)) {
          const content = this.dealData(item.fieldvalue).length > 0 ? '有' : '无';
    ...
    return (
      <div className={styles.default}>
        <ReactTable
          ...
          style={{
            maxWidth: '100%',
            height: tableData.length > 0 ? tableData.length * 38 + 40 : '200px',
            maxHeight: '400px',
            textAlign: 'center'
          }}
        />
