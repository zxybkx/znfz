...
import {Button, Row, Form, Collapse, Input, Tooltip, Drawer, Checkbox, message} from 'antd';
...
export default class MarkInfo extends Component {
  ...
  saveMark = (_data, key, attribute) => {
    const {dispatch, match, fileKey, getMark, filename, fileCategory, imageSource, currentPage} = this.props;
    ...
    dispatch({
      ...
      if (success) {
        ...
        if (!_.isEmpty(_data.attribute) || !_.isEmpty(_data.analysis) || !_.isEmpty(_data.excerpt)) {
          ...
          this.props.addWsMark(currentPage, filename, imageSource, fileCategory, '1')
        }
      }
    })
  };
  ...
  render() {
    ...
    const {markList, factMarkList, fileKey, fileCategory, facts, filename, ysay, currentPage, imageSource} = this.props;
    ...
    const FactItemProps = {
      filename, fileKey, ysay, currentPage, imageSource,
    ...
    const arr = filename.split('（');
    const arr2 = arr.length > 1 ? arr[1].split('）') : [];
                  ...
                  facts && facts.map((obj, idx) => {
                    ...
                    return (
                      <div>
                            ...
                            <div style={{display: 'flex'}}>
                              ...
                              <div className={factStyles.rightCon}>
                                <FormItem className={factStyles.item}>
                                  {
                                    getFieldDecorator('fact' + obj.id,
                                      {
                                        initialValue: obj.rdss,
                                      })(
                                      <TextArea rows={3}
                                                onBlur={(e) => this.saveFact(e, obj.id)}
                                      />,
                                    )}
                                </FormItem>
                              </div>
                        ...
                        <div key={fileKey + idx} className={styles.item}>
                          <div className={styles.left}>
                            <Tooltip placement="right" title={obj.rdss}>
                              <span>{obj.mergekey}</span>
                            </Tooltip>
                            <div>
                              <a onClick={() => this.onFactImport(obj)}>其他摘录及分析</a>
                            </div>
                          </div>
