...
const loopExpand = (result, data) => data && data.map((item) => {
...
const generateExpandKeys = (tree, expandRoot, expandKeys, expandAll) => {
  ...
  if (expandKeys) {
    expandKeys.map((k, i) => {
      if (expandAll) {
        let expandNode = tree && tree.filter((node, index) => {
          return node.id === k;
        });
        ...
        if (item.fieldpath) {
          return (
            <TreeNode key={`node-${item.id}`}
                      title={
                        <ContextMenuTrigger id={problemMenuId} holdToDisplay={-1} item={item} collect={this.collect}>
          ...
          return (
            <TreeNode key={`node-${item.id}`}
                      title={<span style={{fontWeight: 'bold'}}>
                                <Fontawesome
                                  className={classnames(styles.Icon, styles.Folder, item.children.length > 0 ? '' : styles.empty)}
                                  name={isFile ? 'file-image-o' : (folderOpen ? 'folder-open' : 'folder')}/>{title}</span>
                      }
                      data={item}>
              {loop(filteredData(item.children))}
            </TreeNode>
